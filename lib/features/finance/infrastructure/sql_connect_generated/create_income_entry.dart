...
class CreateIncomeEntryVariablesBuilder {
  ...
  CreateIncomeEntryVariablesBuilder(this._dataConnect, {required  this.spaceId,required  this.description,required  this.kind,required  this.paymentMethod,required  this.amountCents,required  this.occurredAt,required  this.competenceMonth,required  this.idempotencyKey,});
  Deserializer<CreateIncomeEntryData> dataDeserializer = (dynamic json)  => CreateIncomeEntryData.fromJson(jsonDecode(json));
  Serializer<CreateIncomeEntryVariables> varsSerializer = (CreateIncomeEntryVariables vars) => jsonEncode(vars.toJson());
  Future<OperationResult<CreateIncomeEntryData, CreateIncomeEntryVariables>> execute() {
    return ref().execute();
  }
